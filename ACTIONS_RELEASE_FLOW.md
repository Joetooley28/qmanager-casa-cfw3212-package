...
Published prereleases also upload one small updater-only changelog asset:

```text
qmanager-cfw3212-vX.Y.Z-cfw3212.N-changelog.json
```

That JSON is generated by the builder from the public curated Casa changelog
source and the exact upstream QManager release body. It is used by the Casa
Software Update page to switch the existing release-notes panel between
